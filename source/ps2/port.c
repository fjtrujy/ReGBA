...
void ReGBA_DisplayFPS(void)
{
	...
	{
		char line[512];
		sprintf(line, "%2i/%3i", Stats.RenderedFPS, Stats.EmulatedFPS);
		// White text, black outline
		
		PrintStringOutline(line, RGB888_TO_RGB565(255, 255, 255), RGB888_TO_RGB565(0, 0, 0), gsTexture.Mem, gsTexture.Width * 2, 7, 3, gsTexture.Width - 14, gsTexture.Height - 6, LEFT, BOTTOM);
	}
}
...
bool ReGBA_GetBundledGameConfig(char* Result)
{
	...
	if (strlen(main_path) + strlen(CONFIG_FILENAME) + 1 /* "/" */ > MAX_PATH)
		return false;

	sprintf(Result, "%s/%s", main_path, CONFIG_FILENAME);
