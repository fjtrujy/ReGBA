 ...
 *  $12    %mm4 -> tmp2
 *  $13    %mm5 -> tmp3
 *  $14    %mm6 -> *current_upper
 *  $15    %mm7 -> *current
 */
static inline void scale2x_16_mmi_border(scale2x_uint16* dst, const scale2x_uint16* src0, const scale2x_uint16* src1, const scale2x_uint16* src2, unsigned count)
{
	assert(count >= 8);
	assert(count % 4 == 0);
...
/* final run */
		...
		: "0" (src0), "1" (src1), "2" (src2), "3" (dst), "4" (count)
		: "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7"
	);
}


void scale2x_16_mmi(scale2x_uint16* dst0, scale2x_uint16* dst1, const scale2x_uint16* src0, const scale2x_uint16* src1, const scale2x_uint16* src2, unsigned count)
{
	if (count % 4 != 0 || count < 8) {
		scale2x_16_def(dst0, dst1, src0, src1, src2, count);
	} else {
		scale2x_16_mmi_border(dst0, src0, src1, src2, count);
		scale2x_16_mmi_border(dst1, src2, src1, src0, count);
	}
}
