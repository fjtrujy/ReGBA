...
int load_cheats_name(char* cheats_msg, char** cheats_msg_pt, unsigned int num)
{
	...
	FILE_OPEN(cheats_file, cheat_path_loaded, READ);
	if(!FILE_CHECK_VALID(cheats_file))
		return -1;
	...
	while(FILE_GETS(current_line, 256, cheats_file))
    {
...
void add_cheats(char *cheats_filename)
{
  ...
  if (DEFAULT_CHEAT_DIR != NULL) {
    sprintf(cheat_path, "%s/%s/%s.cht", main_path, DEFAULT_CHEAT_DIR, cheats_filename);
  }
  else
  {
    sprintf(cheat_path, "%s/%s.cht", main_path, cheats_filename);
  }

  FILE_OPEN(cheats_file, cheat_path, READ);
...
printf("find a valid entrey\n");
        ...
        strncpy(current_cheats_flag[g_num_cheats].cheat_name, name_ptr, CHEAT_NAME_LENGTH - 1);
        current_cheats_flag[g_num_cheats].cheat_name[CHEAT_NAME_LENGTH - 1] = 0;
        cheat_name_length = strlen(current_cheats_flag[g_num_cheats].cheat_name);
        if(cheat_name_length &&
         ((current_cheats_flag[g_num_cheats].cheat_name[cheat_name_length - 1] == '\n') ||
         (current_cheats_flag[g_num_cheats].cheat_name[cheat_name_length - 1] == '\r')))
        {
          current_cheats_flag[g_num_cheats].cheat_name[cheat_name_length - 1] = 0;
          cheat_name_length--;
        }
...
void process_cheats()
{
  ...
  {
    if(current_cheats_flag[i].cheat_active == 1)
    {
      switch(current_cheats_flag[i].cheat_variant)
      {
        case CHEAT_TYPE_GAMESHARK_V1:
        case CHEAT_TYPE_DIRECT_V1:
          process_cheat_gs1(current_cheats_flag + i);
          break;

        case CHEAT_TYPE_GAMESHARK_V3:
        case CHEAT_TYPE_DIRECT_V3:
          process_cheat_gs3(current_cheats_flag + i);
          break;

		case CHEAT_TYPE_CHT:
//printf("cheat%d\n", i);
			process_cheat_cht(current_cheats_flag + i);
		  break;
